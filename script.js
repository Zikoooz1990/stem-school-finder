...
async function init() {
  ...
  try {
    ...
    schoolsListDiv.innerHTML = `<p>Couldn't get your location. Showing distances from Cairo center.</p>`;
  }
  ...
  const distancePromises = schoolsData.map(async school => {
    ...
    try {
      ...
      school.distance = straightLineDistance(userLocation.lat, userLocation.lng, school.lat, school.lng);
      school.note = 'straight line';
    }
    return school;
  });
  ...
  schoolsData.forEach(school=>{
    L.marker([school.lat, school.lng]).addTo(map)
      .bindPopup(`<b>${school.name}</b><br>${school.address}<br>${school.distance.toFixed(2)} km (${school.note})`);
    ...
    div.innerHTML = `
      <h3>${school.name}</h3>
      <p>${school.address}</p>
      <p>Distance: ${school.distance.toFixed(2)} km (${school.note})</p>
